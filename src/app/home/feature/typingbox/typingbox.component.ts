...
export class TypingBoxComponent {
  ...
  private textDisplayChild = viewChild.required('textDisplay', { read: ElementRef })
  private nonResponsiveKeys = [
    'Shift', 'Control', 'Escape', 'CapsLock', 'Alt', 'Meta', 'AltGraph', 'Symbol', 'SymbolLock',
    'Home', 'Delete', 'End', 'Fn', 'FnLock', 'NumLock', 'ScrollLock', 'Hyper', 'Super',
    'ArrowLeft', 'ArrowUp', 'ArrowDown', 'ArrowRight', 'PageDown', 'PageUp', 'Insert'
  ]
  ...
  keyCheck(event: KeyboardEvent): void {
    const key = event.key
    if (this.nonResponsiveKeys.includes(key))
    ...
    switch (key) {
      case 'Backspace':
        ...
        if (this.correctInputChars > 0 && textString[this.letterIndex] === this.inputText[this.letterIndex])
      ...
      case 'Tab': {
        ...
        const tabValue = ' '
        if (textLetter === '\n') return;
        const isCorrectInput = textString[this.letterIndex] === tabValue && textString[this.letterIndex + 1] === tabValue
  ...
  textScroll(event: Event) {
    ...
    const textWindowElement = this.textDisplayChild().nativeElement as HTMLDivElement
